...
file_handler = logging.FileHandler(
    ROOTPATH + "/tests/logs/gatiab.log", mode='w'
)
...
def get_rsrf_data() -> tuple[list[NDArray], list[NDArray]]:
...
def test_gas(
    ...
    if gas == 'H2O':
        gas_content = np.array([3., 3.5, 4.])
    else:
        gas_content = np.array([250., 300., 350.])
    ...
    if gas == 'H2O':
        U = xr.DataArray(np.array([3.5]), dims='U')
    else:
        U = xr.DataArray(np.array([300.]), dims='U')
    ...
    for iband in range(0, nbands):
        ...
        if gas == 'H2O':
            logger.info(
                f"{gas} - {iwvlc} nm - T_ref={H2O_RES[bandname]:.3f} - "
                f"T_calc={t_gas_ib  :.3f}"
            )
            assert (t_gas_ib == H2O_RES[bandname]), (
                f"Problem with {bandname}({iwvlc}) "
                f"get {t_gas_ib} instead of {H2O_RES[bandname]}"
            )
        else:
            logger.info(
                f"{gas} - {iwvlc} nm - T_ref={O3_RES[bandname]:.3f} - "
                f"T_calc={t_gas_ib  :.3f}"
            )
            assert (t_gas_ib == O3_RES[bandname]), (
                f"Problem with {bandname}({iwvlc}) "
