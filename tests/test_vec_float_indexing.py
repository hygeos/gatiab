...
"""
Unit tests for vec_float_indexing (and its helper get_binary_mat).

vec_float_indexing is a port of the multilinear interpolation performed by
LUT.__getitem__ / Idx from the luts package (https://github.com/hygeos/luts)
and must give the same results. It is also used outside gatiab (e.g. the
atmosphere module of the smartg_pv package), hence the exhaustive coverage.

These tests are self-contained: no ckdmip/atmosphere data directories needed.
The oracle for random-data cross-checks is scipy's RegularGridInterpolator
on integer axes (verified during development to match luts to ~1e-16).
"""
...
def rgi_oracle(data, keys_arrays, fill_value=np.nan):
    """
    Reference multilinear interpolation of data at fractional indices,
    one 1-D float array per dimension (no slices).
    """
...
class TestDocstringExamples:
    # Executable version of the docstring examples (doctest collection is not
    # enabled: the docstring reprs are not numpy>=2 clean)

    def test_array_key_with_slice(self, m1):
        res = vec_float_indexing(m1, [np.array([0.8, 1.1, 1.5]), slice(None)])
        assert_allclose(res, [[1.6, 2.6], [2.2, 3.2], [3.0, 4.0]], rtol=1e-12)

    def test_scalar_keys(self, m1):
...
class TestExactGridPoints:
    ...
    def test_integer_valued_array_key(self, m1, key_val):
        res = vec_float_indexing(m1, [np.array([key_val]), slice(None)])
        assert_array_equal(res, m1[[int(key_val)], :])

    @pytest.mark.parametrize('key_val', [0.0, 1.0, 2.0])
    def test_integer_valued_scalar_key(self, m1, key_val):
        res = vec_float_indexing(m1, [key_val, 0.0])
        assert res == m1[int(key_val), 0]

    def test_full_identity(self, m1):
        res = vec_float_indexing(m1, [np.array([0.0, 1.0, 2.0]), slice(None)])
        assert_array_equal(res, m1)


class TestEdgeKeys:
    # key == N-1 raised IndexError before commit a098939; key == 0 covers
    # the other clamp boundary

    def test_array_key_equals_n_minus_1(self, m1):
        res = vec_float_indexing(m1, [np.array([2.0]), slice(None)])
        assert_array_equal(res, [[4.0, 5.0]])

    def test_array_key_equals_zero(self, m1):
        res = vec_float_indexing(m1, [np.array([0.0]), slice(None)])
        assert_array_equal(res, [[0.0, 1.0]])

    def test_scalar_keys_at_upper_edges(self, m1):
        assert vec_float_indexing(m1, [2.0, 1.0]) == 5.0


class TestSlicePositions:
    # A slice placed before an interpolated key must not shift the corner
    # selection of the following dimensions (regression test: bit position
    # among interpolated keys vs raw dimension position, as in luts)

    def test_leading_slice(self, m1):
    ...
    def test_h2o_call_site_pattern(self):
...
class TestScalarKeys:
    ...
    def test_all_scalar_keys_return_scalar(self, m1):
    ...
    def test_scalar_key_with_slice(self, m1):
...
class TestOracleCrossCheck:
    # Property-style check on random data against RegularGridInterpolator,
    # all dimensions indexed with in-range 1-D float arrays

    @pytest.mark.parametrize('shape', [(4, 5), (4, 5, 6), (3, 4, 5, 6)])
    def test_matches_regular_grid_interpolator(self, shape):
...
class TestCallSitePatterns:
    # Shapes and key layouts as used by ckdmip2od with float_indexing='fast'

    def test_3d_trailing_slice(self):
    ...
    def test_keys_exactly_on_last_grid_point(self):
...
class TestDimensionality:
    ...
    def test_1d_data_single_key(self):
    ...
    def test_5d_data_all_array_keys(self):
...
class TestInputsAndDtypes:
    ...
    def test_keys_list_not_mutated(self, m1):
        k0 = np.array([0.8, 1.1])
        k1 = slice(None)
        keys = [k0, k1]
        k0_copy = k0.copy()
    ...
    def test_int_data_returns_float64(self, m1):
        res = vec_float_indexing(m1, [np.array([0.5]), slice(None)])
        assert res.dtype == np.float64

    def test_float32_data_returns_float64(self, m1):
...
class TestGetBinaryMat:
    ...
    def test_rows_are_little_endian_bits(self):
